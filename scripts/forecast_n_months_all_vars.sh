...
# set parameters 
first_month=1 # first initialisation month
last_month=5 # last initialisation month
...
for variable in total_precipitation 2m_temperature; do
    # loop over all initialisation months 
    for month in $(seq $first_month $last_month);do 
        ...
        for centre in meteo_france dwd cmcc ncep ukmo ecmwf jma eccc ;do  
            ...
            python get_any_hindcast.py \
                ...
                --years $years \
                > $logdir/download_log_${variable}_${centre}_${years}_${month}.txt 2>&1
            exitcode=$?
            set -e
            ...
            python forecast_products.py \
                ...
                --yearsfc $years \
                > $logdir/products_log_${variable}_${centre}_${years}_${month}.txt 2>&1
            exitcode=$?
            set -e
            if [ $exitcode -eq 0 ]; then
            ...
            python forecast_plots.py \
                ...
                --plotsdir $plotdir \
                --yearsfc $years \
                > $logdir/plots_log_${variable}_${centre}_${years}_${month}.txt 2>&1
            exitcode=$?
            set -e
            if [ $exitcode -eq 0 ]; then
