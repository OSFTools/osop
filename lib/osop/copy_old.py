...
# ):
#     """
#     Check grib file exists and open

#     grib_file (str): file name for grib file
#     cfgrib_kwards (bool): Key word arguments
#     """

#     file = Path(grib_file)  # Pull full file path
#     if not file.is_file():  # is file checkes if file exists
#         raise FileNotFoundError(
#             errno.ENOENT, os.strerror(errno.ENOENT), grib_file
...
#     ds = xr.open_dataset(  # Open GRIB file using x array, backend
#         grib_file, engine="cfgrib", backend_kwargs=bkw
#     )
#     return ds  # Return grib file as x array dataset
...
#First things first --> open the grib file as a dataset, 
...
def grib_exist(grib_file, cfgrib_kwargs=None):
    ...
    try:
        eccodes.codes_get(gid, "indexingDate")
        st_dim_name = "indexing_time"
        is_lagged = True
        print(f"Detected lagged ensemble (indexing_time)")
    ...
    # For lagged ensembles, specify time dimension mapping
    if is_lagged:
    ...
    rename_dict = {}
    if is_lagged:
        # Lagged: indexing_time → time, forecastMonth → step
    ...
    # For lagged ensembles, rebuild valid_time with correct dimensions
    if is_lagged:
        print("is lagged here")
        ...
        if has_step and has_time:
            print("  → Reconstructing valid_time from time + step...")
            ...
            print(f"    valid_time shape: {ds.valid_time.shape}, dims: {ds.valid_time.dims}")
            print(f"    First row valid_time: {ds.valid_time.values[0] if len(valid_time_dims) > 1 else ds.valid_time.values}")
...
def _prepare_data(ds):
    ...
    ren = {}
    if "latitude" in F.coords:  # Rename coorids lat/lon x/y
        ren["latitude"] = "Y"
...
def process_grib_to_pycpt(
    ...
    if cast == "hindcast":
        ...
        cast_bname = "{origin}_{systemfc}_{fcstarty}-{fcendy}_monthly_mean_{start_month}_{leads_str}_{area_str}_{hc_var}".format(
            **config
        )
        cast_fname = f"{downloaddir}/{cast_bname}.grib"
    elif cast == "obs":

        cast_bname = "predictand_era5_{var}_{hcstarty}-{hcendy}_monthly_{start_month}_{leads_str}_{predictand_string}".format(
            **config
        )
        cast_fname = f"{downloaddir}/{cast_bname}.grib"
    else:
