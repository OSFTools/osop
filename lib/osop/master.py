...
def process_centre(centre, month, leads, area, variable, downloaddir, productsdir, 
                   scoresdir, plotdir, logdir, location, method):
    ...
    if centre != "mme":
        ...
        success = run_python_script('get_any_hindcast.py', args, 
                                    f"{centre} : download")
        if not success:
    ...
    args = [
        ...
        '--variable', variable,
        '--downloaddir', str(downloaddir),
        '--productsdir', str(productsdir),
        '--logdir', str(logdir)
    ]
    success = run_python_script('compute_products.py', args, 
                                f"{centre} : products generation")
    ...
    args = [
        ...
        '--area', area,
        '--downloaddir', str(downloaddir),
        '--scoresdir', str(scoresdir),
        '--productsdir', str(productsdir),
        '--variable', variable,
        '--logdir', str(logdir)
    ]
    success = run_python_script('compute_scores.py', args, 
                                f"{centre} : scores generation")
    ...
    success = run_python_script('plot_verification.py', args, 
                                f"{centre} : plots generation")
...
def main():
    ...
    # Get ERA5 data
    get_era5_data(month, leads, area, dirs['downloaddir'], 
                  dirs['logdir'], variable)
    
    # List of centres to process
    centres = ['meteo_france', 'dwd', 'cmcc', 'ncep', 'ukmo', 
               'ecmwf', 'jma', 'eccc', 'mme']
