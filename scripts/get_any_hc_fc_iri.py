...
def get_iri_nnme(
    ...
    for variable in VARIABLES:
        if l_fc:
            ...
            da = cptdl.download(
                template, destination_file, use_dlauth=False, verbose=False, **kwargs_fc
            )
            ...
            da = cptdl.download(
                template, destination_file, use_dlauth=False, verbose=False, **kwargs_hc
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--variable", required=True, help="variable for download PRCP or T2M"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--forecast",
        required=False,
        help="Get forecast? If false, get hindcast",
        action="store_true",
    )
